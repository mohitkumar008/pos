$(document).ready(function () {
    ...
    $('button#pos-finalize').click(function () {
        // Pos Changes start
        if ($('#payment_rows_div .col-md-12').length) {
            $('#payment_rows_div .col-md-12').remove();
        }
        $('button#add-payment-row').trigger('click');
        // Pos Changes ends
    ...
    $('button#add-payment-row').click(function () {
        ...
        $.ajax({
            ...
            data: { row_index: row_index, location_id: location_id, customer_id: customer_id, first_payment_type: first_payment_type },
            dataType: 'html',
            success: function (result) {
                if (result) {
                    ...
                    var first_payment_type = $('#payment_rows_div')
                        .find('.payment_types_dropdown')
                        .first()
                        .val()
                    if (first_payment_type == "custom_pay_1" || first_payment_type == "custom_pay_2" || first_payment_type == "custom_pay_3" || first_payment_type == "custom_pay_4") {
        ...
        swal({
            ...
            if (willDelete) {
                $(this)
                    ...
                    .remove();
                calculate_balance_due();
                $('.reset_payment_modal').trigger('click');
                $("#payment_rows_div:first-child .payment-amount").val(discount).trigger('change').attr('disabled', false);
    ...
    $(document).on('change', '.wallet_otp', function () {
        event.preventDefault();

        var otp = $(this).val()
        var url = 'https://armadamart.com/api/user/validate-wallet-otp'
...
$(document).on('click', '.reset_payment_modal', function (e) {
    let total_payable = __read_number($('input#final_total_input'));

    $("#payment_rows_div:first-child .payment-amount").val(total_payable).trigger('change');

    $("#payment_rows_div:first-child")
        .children()
        .not(':first-child')
        .remove();
...
$(document).on('change', '#payment_rows_div .col-md-12:nth-child(1) .payment_types_dropdown', function (e) {
    ...
    // For cash & other payment method

    if ($('#payment_rows_div .col-md-12:first-child .payment_types_dropdown').val() == 'cash' || $('#payment_rows_div .col-md-12:first-child .payment_types_dropdown').val() == 'other') {
        $("#payment_rows_div:first-child .payment-amount").trigger('change').attr('readonly', false);
        if ($('.remove_payment_row').length > 0) {
            $('.remove_payment_row').closest('.payment_row').parent().remove();
            $("#payment_rows_div:first-child .payment-amount").val(total_payable);
        }
        calculate_balance_due();
    }
...
function send_wallet_otp(walletType, member_id) {
    $('.send-wallet-otp').hide();
    $('.otp-timer').show();
    timerOn = true;
    timer(5);
    ...
    $.ajax({
        type: 'POST',
        url: 'https://armadamart.com/api/user/send-wallet-otp',
