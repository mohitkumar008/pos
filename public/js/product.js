...
//get customer
    $('select#products_for_location').select2({
        ajax: {
            url: '/products_for_location',
            dataType: 'json',
            delay: 250,
        ...
        language: {
            noResults: function () {
                var name = $('#products_for_location')
                    .data('select2')
                    .dropdown.$search.val();
                return (
                    '<button type="button" data-name="' +
                    name +
                    '" class="btn btn-link add_quick_product"><i class="fa fa-plus-circle fa-lg" aria-hidden="true"></i>&nbsp; ' +
                    __translate('product.add_new_product', { name: name }) +
                    '</button>'
                );
            },
        },
    ...
    //Quick add product
    $(document).on('click', 'button.add_quick_product', function () {
