...
$(document).on('click', 'button#submit_purchase_form', function (e) {
    ...
    if ($('table#purchase_entry_table tbody tr').length <= 0 && !$('#purchase_csv').val()) {
...
function toggle_search() {
    if ($('#location_id').val()) {
        ...
        $('#purchase_csv').removeAttr('disabled');
        $('#search_product').focus();
    } else {
        $('#search_product').attr('disabled', true);
        $('#purchase_csv').attr('disabled', true);
    }
}
...
$('#uploadPurchaseData').on('click', function (e) {
    $('#loading-gif').show();
    $('#file_import_error').html('');
    var row_count = $('#row_count').val();
    var location_id = $('#location_id').val();
    var file_data = $('#purchase_csv').prop('files')[0];
    if (file_data != undefined) {
        var form_data = new FormData();
        form_data.append('purchase_csv', file_data);
        form_data.append('location_id', location_id);
        form_data.append('row_count', row_count);
        $.ajax({
            type: 'POST',
            url: '/purchases/upload_purchase_data',
            contentType: false,
            processData: false,
            data: form_data,
            success: function (response) {
                if(response.success == 0) {
                    ...
                    <strong><span>Error :- </span>${response.msg}</strong>
                    </div>
                    `;
