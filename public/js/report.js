$(document).ready(function() {
    ...
    //Date range as a button
    $('#stock_report_date_filter').daterangepicker(
        dateRangeSettings,
        function (start, end) {
            $('#stock_report_date_filter').val(start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format));
            stock_report_table.ajax.reload();
        }
    );
    $('#stock_report_date_filter').on('cancel.daterangepicker', function(ev, picker) {
        $('#stock_report_date_filter').val('');
        stock_report_table.ajax.reload();
    });
    ...
    stock_report_table = $('#stock_report_table').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                var start = '';
                var end = '';
                if ($('#stock_report_date_filter').val()) {
                    start = $('input#stock_report_date_filter')
                        .data('daterangepicker')
                        .startDate.format('YYYY-MM-DD');
                    end = $('input#stock_report_date_filter')
                        .data('daterangepicker')
    ...
    if ($('#product_sr_date_filter').length == 1) {
        $('#product_sr_date_filter').daterangepicker(
            ...
            function(start, end) {
                ...
                product_sell_report_with_tax_report.ajax.reload();
    ...
    product_sell_report = $('table#product_sell_report_table').DataTable({
        ...
        columns: [
            ...
            { data: 'unit_price_inc_discount', name: 'unit_price_inc_discount' },
            { data: 'igst_tax', name: 'igst_tax'},
            { data: 'igst_amount', name: 'igst_amount'},
            { data: 'cgst_tax', name: 'cgst_tax'},
            { data: 'cgst_amount', name: 'cgst_amount'},
            { data: 'sgst_tax', name: 'sgst_tax'},
    ...
    product_sell_report_with_tax_report = $('table#product_sell_report_with_tax_table').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[3, 'desc']],
        aLengthMenu: [[25, 50, 100, 200, 500,1000,2000,5000], [25, 50, 100, 200, 500,1000,2000,5000]],
        ajax: {
            url: '/reports/product-sell-report-with-tax',
            data: function(d) {
                var start = '';
                var end = '';
                var start_time = $('#product_sr_start_time').val();
        ...
        columns: [
            ...
            { data: 'igst_tax', name: 'igst_tax', "bSortable": true},
            { data: 'igst_amount', name: 'igst_amount', "bSortable": true},
            { data: 'cgst_tax', name: 'cgst_tax', "bSortable": true},
            { data: 'cgst_amount', name: 'cgst_amount', "bSortable": true},
            { data: 'sgst_tax', name: 'sgst_tax', "bSortable": true},
            { data: 'sgst_amount', name: 'sgst_amount', "bSortable": true},
            { data: 'cess_tax', name: 'cess_tax', "bSortable": true},
            { data: 'cess_amount', name: 'cess_amount', "bSortable": true},
            { data: 'item_tax', name: 'item_tax' },
            // { data: 'unit_sale_price', name: 'transaction_sell_lines.unit_price_inc_tax' },
            { data: 'subtotal', name: 'subtotal', searchable: false },
        ],
        fnDrawCallback: function(oSettings) {
            $('#footer_tax_subtotal').text(
                sum_table_col($('#product_sell_report_with_tax_table'), 'row_subtotal')
            );
            $('#footer_tax_total_sold').html(__sum_stock($('#product_sell_report_with_tax_table'), 'sell_qty'));
            $('#footer_tax_total').html(__sum_stock($('#product_sell_report_with_tax_table'), 'tax', 'left'));
            __currency_convert_recursively($('#product_sell_report_with_tax_table'));
        },
    });
