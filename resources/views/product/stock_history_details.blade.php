			...
			<tr>
				...
				<td>
				    <div class="current_stock_row">
				        <span class="display_currency text-primary" data-is_quantity="true" id="current_stock">{{$stock_details['current_stock']}}</span> {{$stock_details['unit']}} <span id="current_stock_input"></span>
				    </div>
				</td>
			</tr>
		</table>
		...
		<table class="table table-slim" id="stock_history_table">
			...
			@forelse($stock_history as $k=>$history)
				<tr>
					...
					<td><span class="display_currency" data-is_quantity="true" id="new_quantity_{{$k}}">{{$history['stock']}}</span></td>
