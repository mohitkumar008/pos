    ...
    <body>
        <div class="ticket">
                	...
                	@forelse($receipt_details->lines as $line)
	                    <tr>
	                        ...
	                        <td class="description" colspan="4">
	                        	@forelse($line['tax_details'] as $taxes)
    	                        	@if($taxes['name']!='IGST')
    	                        	    {{$taxes['name']}} @ {{$taxes['amount']}}%,
    	                        	@endif
	                        	@endforeach
	                        </td>
	                    </tr>
            ...
            @if(empty($receipt_details->hide_price) && !empty($receipt_details->tax_summary_label) )
	            <!-- tax -->
	            @if(count($receipt_details->taxes))
                <table  class="border-bottom width-100 table-f-12 mb-10">
                    <tr class="bg-green">
                        <th class=" text-center">@lang('HSN')</th>
                        <th class=" text-center">@lang('Taxable')</th>
                        <th class=" text-center">@lang('CGST')</th>
                        <th class=" text-center">@lang('SGST')</th>
                        <th class=" text-center">@lang('CESS')</th>
                        <th class=" text-center">@lang('Total Amt')</th>
                    </tr>
                    ...
                    @foreach($receipt_details->taxes as $key => $tax)
                        @php
                            $total_taxable_amount+=isset($tax['taxable_amount'])?$tax['taxable_amount']:0;
                            $CGST_total+=isset($tax['CGST_total_tax'])?$tax['CGST_total_tax']:0;
                            $SGST_total+=isset($tax['SGST_total_tax'])?$tax['SGST_total_tax']:0;
                            $CESS_total+=isset($tax['CESS_total_tax'])?$tax['CESS_total_tax']:0;
                            $total_tax+=isset($tax['total_tax'])?$tax['total_tax']:0;
                        @endphp
                        <tr class="bg-green">
                            <td class=" text-center">{{$key}}</td>
                            <td class=" text-center">@if(isset($tax['taxable_amount'])) {{$tax['taxable_amount']}} @else - @endif</td>
                            <td class=" text-center">@if(isset($tax['CGST_total_tax'])) {{$tax['CGST_total_tax']}} @else - @endif</td>
                            <td class=" text-center">@if(isset($tax['SGST_total_tax'])) {{$tax['SGST_total_tax']}} @else - @endif</td>
                            <td class=" text-center">@if(isset($tax['CESS_total_tax'])) {{$tax['CESS_total_tax']}} @else - @endif</td>
                            <td class=" text-center">@if(isset($tax['total_tax'])) {{$tax['total_tax']}} @else - @endif</td>
                        </tr>
                    @endforeach
                    <tr class="bg-green">
                            <td class=" text-center">Total</td>
                            <td class=" text-center">@if(isset($total_taxable_amount)) {{$total_taxable_amount}} @else - @endif</td>
                            <td class=" text-center">@if(isset($CGST_total)) {{$CGST_total}} @else - @endif</td>
                            <td class=" text-center">@if(isset($SGST_total)) {{$SGST_total}} @else - @endif</td>
                            <td class=" text-center">@if(isset($CESS_total)) {{$CESS_total}} @else - @endif</td>
                            <td class=" text-center">@if(isset($total_tax)) {{$total_tax}} @else - @endif</td>
                        </tr>
                </table>
                @endif
            @endif
