...
<div class="payment_details_div @if( $payment_line['method'] !== 'custom_pay_' . $i ) {{ 'hide' }} @endif" data-type="custom_pay_{{$i}}" >
	<div class="col-md-12">
		<div class="form-group">
			{!! Form::label("transaction_no_{$i}_{$row_index}", __('OTP')) !!}
			{!! Form::text("payment[$row_index][transaction_no_{$i}]", $payment_line['transaction_no'], ['class' => 'form-control wallet_otp', 'placeholder' => __('OTP'), 'id' => "transaction_no_{$i}_{$row_index}"]); !!}
		</div>
	</div>
	<div class="col-md-6 text-left">
        <a href="javascript:" class="send-wallet-otp" data-payment_type="custom_pay_{{$i}}">Resend OTP</a>
