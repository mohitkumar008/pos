        ...
        <div class="modal-body">
            <div class="row">
                <div class="form-group col-xs-12 @if (!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
                    ...
                    <input type="input" name="products[{{ $row_count }}][unit_price]"
                        class="form-control pos_unit_price input_number mousetrap" readonly
                ...
                <div class="form-group col-xs-12 col-sm-6 @if (!$edit_discount) hide @endif ">
                    ...
                    {!! Form::hidden("products[$row_count][default_line_discount_amount]", @num_format($discount_amount), [
                        'class' => 'form-control input_number default_row_discount_amount',
                    ]) !!}
                    {!! Form::hidden("products[$row_count][default_line_discount_type]", $discount_type, [
                        'class' => 'form-control input_number default_row_discount_type',
                    ]) !!}
                </div>
                {{-- @if (!empty($discount))
					<div class="form-group col-xs-12">
                ...
                @if (!$all_discount->isEmpty())
                    <div class="form-group col-xs-12">
                        ...
                        {!! Form::select("products[$row_count][all_discount]", $all_discount, $discount->id, [
                            'placeholder' => 'Select',
                            'class' => 'form-control all_discount',
                        ]) !!}
                    </div>
                @endif
