...
class Category extends Model
{
    ...
    public static function catAndSubCategories($business_id)
    {
        $all_categories = Category::where('business_id', $business_id)
                                ->where('category_type', 'product','category_id')
                                ->orderBy('name', 'asc')
                                ->get()
                                ->toArray();
        ...
        if (!empty($sub_categories)) {
            foreach ($sub_categories as $sub_category) {
                if (empty($sub_cat_by_parent[$sub_category['category_id']])) {
                    $sub_cat_by_parent[$sub_category['category_id']] = [];
                }

                $sub_cat_by_parent[$sub_category['category_id']][] = $sub_category;
            }
        }
