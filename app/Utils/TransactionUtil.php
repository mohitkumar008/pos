...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'total_before_tax' => $invoice_total['total_before_tax']-$invoice_total['tax'],
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        if (!empty($lines_formatted)) {
            ...
            // dd($products_modified_combo);
            foreach ($lines_formatted as $key => $value) {
                if (!empty($products_modified_combo[$key]['product_type']) && isset($products_modified_combo[$key]['combo']) && $products_modified_combo[$key]['product_type'] == 'combo') {
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            foreach($lines as $sell_lines){
                
                $tax = $sell_lines->product->product_tax;
                
                if (!empty($tax)) {
                    $tax_name = $tax->name;
                    if (!isset($output_taxes[$tax_name]['taxable_amount'])) {
                        $output_taxes[$tax_name]['taxable_amount'] = 0;
                    }
                    $output_taxes[$tax_name]['taxable_amount'] += $sell_lines->quantity * $sell_lines->unit_price;
                    
                    if (!isset($output_taxes[$tax_name]['total_tax'])) {
                        $output_taxes[$tax_name]['total_tax'] = 0;
                    }
                    $output_taxes[$tax_name]['total_tax'] +=  $sell_lines->item_tax;
                    
                    if ($tax) {
                        $group_tax_details = $this->groupTaxDetails($tax, $sell_lines->quantity * $sell_lines->item_tax);
                        
                        foreach ($group_tax_details as $key => $value) {
                            if (!isset($output_taxes[$tax_name][$value['name']])) {
                                $output_taxes[$tax_name][$value['name']] = 0;
                            }
                            $output_taxes[$tax_name][$value['name']] = $value;
                            $output_taxes[$tax_name][$value['name']."_total_tax"] += $value['calculated_tax'];
                        }
                    }
                    // else {
            ...
            foreach ($details['lines'] as $line) {
                ...
                $total_tax += (float) $line['tax']* (float)$line['quantity_uf'];
            ...
            $output['subtotal_exc_tax'] = $this->num_f($subtotal_exc_tax + $total_line_discount, true, $business_details, true);
        ...
        $output['total_tax'] = $this->num_f($total_tax, true, $business_details, true);
        ...
        if ($transaction->tax_amount != 0 && $transaction->tax_id!=null && $tax->is_tax_group) {
                        ...
                        for ($i=1; $i<8; $i++) { 
                            if ($value['method'] == "custom_pay_{$i}") {
                                $output['payments'][] =
                                    ['method' => $method,
                                    'amount' => $this->num_f($value['amount'], $show_currency, $business_details),
                                    'transaction_id' => (!empty($value['transaction_no']) ? (trans("lang_v1.transaction_no") . ':' . $value['transaction_no']) : ''),
                                    'date' => $this->format_date($value['paid_on'], false, $business_details)
                                    ];
                            }
                        }
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            if (in_array('invoice_url', $qr_code_fields)) {
                $qr_code_details[] = '<a href="'.$this->getInvoiceUrl($transaction->id, $business_details->id).'">'.$this->getInvoiceUrl($transaction->id, $business_details->id).'</a>';
            }
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $sell_price_inc_tax = $line->variations->sell_price_inc_tax;
            ...
            $line_array = [
                ...
                'tax_id' => $line->tax_id,
                'tax_details' => $this->groupTaxDetails($line->tax_id, $line->item_tax),
                ...
                'sell_price_inc_tax' => $this->num_f($sell_price_inc_tax, false, $business_details),
                'sell_price_inc_tax_uf' => $sell_price_inc_tax,
        ...
        return ['lines' => $output_lines, 'taxes' => $output_taxes['taxes']];
    }
            ...
            if($tax->amount){
                foreach ($sub_taxes as $sub_tax) {
                    $details[] = [
                        ...
                        'calculated_tax' => ($amount / $tax->amount) * $sub_tax->amount,
                    ];
                }
            }
