...
class ProductUtil extends Util
{
    ...
    public function getProductStockDetails($business_id, $filters, $for)
    {
        ...
        if (!empty($filters['start_date']) && !empty($filters['end_date'])) {
            ...
            $opening_date = $filters['start_date'] . ' 23:59:59';
        ...
        $opening_stock = "(SELECT SUM(OPL.quantity - OPL.quantity_returned) FROM transactions
                            ...
                            WHERE (transactions.status='received' AND transactions.type='opening_stock')  AND transactions.location_id=vld.location_id AND OPL.variation_id=variations.id AND `transactions`.`transaction_date` <= '$opening_date')";
