...
class ProductUtil extends Util
{
     ...
     * @return boolean
     */
    public function updateProductQuantity($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true, $type = null)
    {
        if ($uf_data) {
    ...
    public function calculateInvoiceTotal($products, $tax_id, $discount = null, $uf_number = true)
    {
        ...
        foreach ($products as $product) {
            ...
            $product_total_tax += $quantity * $item_tax;
    ...
    public function getProductStockDetails($business_id, $filters, $for)
    {
        ...
        if (!empty($filters['start_date']) && !empty($filters['end_date'])) {
            ...
            $date_filter = " AND (`transactions`.`transaction_date` BETWEEN '$start_date' AND '$end_date')";
        }
        ...
        $opening_stock = "(SELECT SUM(OPL.quantity - OPL.quantity_returned) FROM transactions 
                            JOIN purchase_lines AS OPL ON transactions.id=OPL.transaction_id
                            WHERE (transactions.status='received' AND transactions.type='opening_stock')  AND transactions.location_id=vld.location_id AND OPL.variation_id=variations.id AND `transactions`.`transaction_date` <= '$start_date 23:59:59')";

        $purchase_qty = "(SELECT SUM(PL.quantity - PL.quantity_returned) FROM transactions 
                            JOIN purchase_lines AS PL ON transactions.id=PL.transaction_id
                            WHERE (transactions.type='purchase_return' OR transactions.type!='opening_stock') AND transactions.location_id=vld.location_id AND PL.variation_id=variations.id $date_filter)";

        $sell_qty = "(SELECT SUM(SL.quantity - SL.quantity_returned) FROM transactions 
                        ...
                        AND SL.variation_id=variations.id $date_filter)";

        $total_sold = "(SELECT SUM(TSL.quantity - TSL.quantity_returned) FROM transactions 
                        JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                        WHERE transactions.status='final' AND transactions.type='sell' AND transactions.location_id=vld.location_id
                        AND TSL.variation_id=variations.id)";

        $total_transfered = "(SELECT SUM(IF(transactions.type='sell_transfer', TSL.quantity, 0) ) FROM transactions 
                                JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                                WHERE transactions.status='final' AND transactions.type='sell_transfer' AND transactions.location_id=vld.location_id AND (TSL.variation_id=variations.id) $date_filter)";

        $total_adjusted = "(SELECT SUM(IF(transactions.type='stock_adjustment', SAL.quantity, 0) ) FROM transactions 
                            ...
                            AND (SAL.variation_id=variations.id) $date_filter)";

        $products = $query->select(
            // DB::raw("(SELECT SUM(quantity) FROM transaction_sell_lines LEFT JOIN transactions ON transaction_sell_lines.transaction_id=transactions.id WHERE transactions.status='final' $location_filter AND
            //     transaction_sell_lines.product_id=products.id) as total_sold"),

            DB::raw("$sell_qty as sell_qty"),

            DB::raw("$opening_stock as openingstock"),
            
            DB::raw("$purchase_qty as purchase_qty"),


            DB::raw("$total_sold as total_sold"),

            DB::raw("$total_transfered as total_transfered"),

            DB::raw("$total_adjusted as total_adjusted"),

            // DB::raw("(SELECT SUM( COALESCE($opening_stock + pl.quantity - ($pl_query_string + $sell_qty + $total_transfered), 0) * purchase_price_inc_tax) FROM transactions 
            //     JOIN purchase_lines AS pl ON transactions.id=pl.transaction_id
            //     WHERE (transactions.status='received' OR transactions.type='purchase_return') AND transactions.type!='opening_stock'  AND transactions.location_id=vld.location_id AND (pl.variation_id=variations.id) $date_filter) as stock_price"),
    ...
    public function productsForLocation($request){
        ...
        if (request()->ajax()) {
            ...
            $products = Product::where('business_id', $business_id);
            if (!empty($term)) {
                $products->where('name', 'like', '%' . $term .'%')->orWhere('sku', 'like', '%' . $term .'%');
            }
            $products->select( 'id','name', 'sk');
