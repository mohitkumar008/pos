...
class ReportController extends Controller
{
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $filters = request()->only(['location_id', 'category_id', 'sub_category_id', 'brand_id', 'unit_id', 'tax_id', 'type', 
                'only_mfg_products', 'active_state',  'not_for_selling', 'repair_model_id', 'product_id', 'active_state', 'start_date', 'end_date']);
            ...
            $datatable =  Datatables::of($products)
                ->editColumn('openingstock', function ($row) {
                    $openingstock = 0;
                    if ($row->openingstock) {
                        $openingstock =  (float)$row->openingstock;
                    }
                    return '<span data-is_quantity="true" class="display_currency opening_stock" data-currency_symbol=false data-orig-value="' . $openingstock . '" data-unit="' . $row->unit . '" >' . $openingstock . '</span> ' . $row->unit;
                })
                ->editColumn('purchase_qty', function ($row) {
                    ...
                    return '<span data-is_quantity="true" class="display_currency purchase_qty" data-currency_symbol=false data-orig-value="' . $purchase_qty . '" data-unit="' . $row->unit . '" >' . $purchase_qty . '</span> ' . $row->unit;
                })
                ->editColumn('sell_qty', function ($row) {
                    ...
                    return '<span data-is_quantity="true" class="display_currency sell_qty" data-currency_symbol=false data-orig-value="' . $sell_qty . '" data-unit="' . $row->unit . '" >' . $sell_qty . '</span> ' . $row->unit;
                })
                ->editColumn('est_qty', function ($row) {
                    $est_qty = ($row->openingstock + $row->purchase_qty)-($row->sell_qty + $row->total_transfered + $row->total_adjusted);
                    return  '<span data-is_quantity="true" class="est_qty display_currency" data-orig-value="' . (float)$est_qty . '" data-unit="' . $row->unit . '" data-currency_symbol=false > ' . (float)$est_qty . '</span>' . ' ' . $row->unit ;
                })
                ...
                ->editColumn('stock_price', function ($row) {
                    $stock = $row->stock ? $row->stock : 0 ;
                    $stock_price = $stock*$row->dpp_inc_tax;
                ...
                ->addColumn('potential_profit', function ($row) {
                    ...
                    $stock_price_by_pp = $stock*$row->dpp_inc_tax;
            ...
            $raw_columns  = ['unit_price', 'total_transfered', 'total_sold',
                    'total_adjusted', 'stock', 'stock_price', 'stock_value_by_sale_price', 'potential_profit','openingstock','purchase_qty','sell_qty','est_qty'];
            ...
            return Datatables::of($query)
                ...
                ->editColumn('discount_amount', '
                    @if($discount_type == "percentage")
                        {{number_format($unit_price*$discount_amount/100,2)}}
                    @elseif($discount_type == "fixed")
                        {{number_format($discount_amount)}}
                    @endif
                    ')
                ->editColumn('item_tax', function ($row) {
                    return number_format($row->item_tax*($row->sell_qty?$row->sell_qty:1),2);
                })
                ->addColumn('unit_price_inc_discount', '{{@number_format($unit_price-($discount_type == "percentage"?($unit_price*$discount_amount/100):$discount_amount),2)}}')
                ->addColumn('igst_amount',function ($row) {
                    // return number_format($row->item_tax/($row->sell_qty?$row->sell_qty:1),2);
                    return '-';
                })
                ->addColumn('igst_tax',function ($row) {
                //   return $row->tax_rate?$row->tax_rate.' %':0;
                    return '-';
                })
                ->addColumn('cgst_amount',function ($row) {
                    return number_format($row->item_tax/(($row->sell_qty?$row->sell_qty:1)*2),2);
                })
                ->addColumn('cgst_tax',function ($row) {
                   return $row->tax_rate?($row->tax_rate/2).' %':0;
                })
                ->addColumn('sgst_amount',function ($row) {
                    return number_format($row->item_tax/(($row->sell_qty?$row->sell_qty:1)*2),2);
                })
                ->addColumn('sgst_tax',function ($row) {
                   return $row->tax_rate?($row->tax_rate/2).' %':0;
                })
                ->editColumn('customer', '@if(!empty($supplier_business_name)) {{$supplier_business_name}},<br>@endif {{$customer}}')
                ->rawColumns(['invoice_no', 'unit_sale_price', 'subtotal', 'sell_qty', 'discount_amount', 'unit_price', 'tax_rate', 'customer','unit_price_inc_discount','igst_tax','igst_amount','cgst_tax','cgst_amount'],'sgst_tax','sgst_amount')
                ->make(true);
        }
     ...
     * @return \Illuminate\Http\Response
     */
    public function getProductSellReportWithTax(Request $request)
    {
        if (!auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->select(
                    ...
                    't.invoice_no',
                    't.transaction_date as transaction_date',
                    DB::raw('sum(transaction_sell_lines.unit_price_before_discount*(transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned)) as unit_price_before_discount'),
                    DB::raw('sum(transaction_sell_lines.unit_price*(transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned)) as unit_price'),
                    DB::raw('sum(transaction_sell_lines.unit_price_inc_tax*(transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned)) as unit_sale_price'),
                    ...
                    DB::raw('sum(transaction_sell_lines.item_tax) as total_tax'),
                    'tax_rates.amount as tax_rate',
            ...
            return Datatables::of($query)
                ...
                ->addColumn('igst_amount',function ($row) {
                    ...
                    $tax = TaxRate::find($row->tax_id);
                    $tax_rate = 0;
                    if(is_object($tax)){
                        $igst = $tax->sub_taxes->where('name', 'IGST')->first();
                        $tax_rate = $igst?$igst->amount:0;
                    }
                    // return number_format(($row->unit_price*$tax_rate)/100,2);
                    return "-";
                })
                ->addColumn('igst_tax',function ($row) {
                ...
                ->addColumn('cgst_amount',function ($row) {
                    ...
                    $tax_rate = 0;
                    if(is_object($tax)){
                        $cgst = $tax->sub_taxes->where('name', 'CGST')->first();
                        $tax_rate = $cgst?$cgst->amount:0;
                    }
                    return number_format(($row->unit_price*$tax_rate)/100,2);
                })
                ->addColumn('cgst_tax',function ($row) {
                ...
                ->addColumn('sgst_amount',function ($row) {
                    ...
                    $tax_rate = 0;
                    if(is_object($tax)){
                        $cgst = $tax->sub_taxes->where('name', 'SGST')->first();
                        $tax_rate = $cgst?$cgst->amount:0;
                    }
                    return number_format(($row->unit_price*$tax_rate)/100,2);
                })
                ...
                ->addColumn('cess_amount',function ($row) {
                    
                    $tax = TaxRate::find($row->tax_id);
                    $tax_rate = 0;
                    if(is_object($tax)){
                        $cess = $tax->sub_taxes->where('name', 'CESS')->first();
                        $tax_rate = $cess?$cess->amount:0;
                    }
                    return number_format(($row->unit_price*$tax_rate)/100,2);
                })
                ->addColumn('cess_tax',function ($row) {
                ...
                ->editColumn('customer', '@if(!empty($supplier_business_name)) {{$supplier_business_name}},<br>@endif {{$customer}}')
                ->rawColumns(['invoice_no', 'unit_sale_price', 'subtotal', 'sell_qty', 'discount_amount', 'unit_price', 'tax_rate', 'customer','unit_price_inc_discount','igst_tax','igst_amount','cgst_tax','cgst_amount','sgst_tax','sgst_amount','cess_tax','cess_amount'])
                ->make(true);
        }
    }
