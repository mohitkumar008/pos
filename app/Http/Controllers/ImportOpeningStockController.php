...
class ImportOpeningStockController extends Controller
{
        ...
        try {
            ...
            if ($request->hasFile('products_csv')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    $filters['product_id'] = $product_info->id;
                    
                    $stock = $this->productUtil->getProductStockDetails($business_id, $filters, 'view_product');
                    
                    // Increase quantity if stock is negative
                    if(!empty($stock[0])){
                        $quantity = $stock[0]->stock < 0 ?  trim($value[2]) + abs($stock[0]->stock) : trim($value[2]);
                    }else{
    ...
    private function addOpeningStock($opening_stock, $product, $business_id, $unit_cost_before_tax, $transaction = null)
    {
        ...
        if($transaction->purchase_lines()->first()){
            //Update purchase line
            $purcahseline['quantity'] = $opening_stock['quantity']+$transaction->purchase_lines()->first()->quantity;
            $transaction->purchase_lines()->update($purcahseline);
                        
        }else{
            //Create purchase line
            $purcahseline['quantity'] = $opening_stock['quantity'];
            $transaction->purchase_lines()->create($purcahseline);

        }
        //Update variation location details
        $this->productUtil->updateProductQuantity($opening_stock['location_id'], $product->id, $product->variation_id, $opening_stock['quantity'], $type = 'opening_stock');

        // Update product prices
        $variation = Variation::find($product->variation_id);

        $variation->default_purchase_price = $unit_cost_before_tax;
        $variation->dpp_inc_tax = $unit_cost_before_tax + $item_tax;
        $profit_percent = $this->productUtil->calc_percentage($unit_cost_before_tax + $item_tax, $variation->profit_percent);
        $variation->default_sell_price = $unit_cost_before_tax + $profit_percent;
        $variation->sell_price_inc_tax = $unit_cost_before_tax + $item_tax + $profit_percent;
        $variation->save();
    }
}
